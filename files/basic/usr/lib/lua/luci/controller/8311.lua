	...
	return {
		{
			...
			items = {
				...
				},
				{
					...
					description = translate(
						"Modify the configured MIB file to set the Version field of any Circuit Pack MEs [6] to match the Hardware Version (if set)."
					),
					type = "checkbox",
					default = false,
				},
				...
				},
				{
					...
					description = translate(
						"PCRE pattern match for automatic updating of Software Versions when OLT uploads a firmware upgrade. Must contain a single sub-pattern match."
					),
					type = "text",
					maxlength = "255",
					base64 = true,
				},
				{
					...
					description = translate(
						"If there are multiple matches for the Firmware Version Match pattern, use this specific match number."
					),
					type = "number",
					min = 1,
					max = 99,
				...
				},
				{
					...
					description = translate(
						"Override which software bank is marked as active in the Software image MEs [7]."
					),
					type = "select",
					default = "",
					options = {
				...
				},
				{
					...
					description = translate(
						"Override which software bank is marked as committed in the Software image MEs [7]."
					),
					type = "select",
					default = "",
					options = {
				...
				},
				{
					...
					description = translate(
						"PON mode of operation. This is where you can choose between XGS-PON (the default) or XG-PON."
					),
					type = "select_named",
					default = "xgspon",
					options = {
					...
					id="reg_id",
					name=translate("Registration ID (Text)"),
					description=translate("Registration ID (up to 36 characters) sent to the OLT, in hex format. This is where you would set a ploam password (which is contained in the last 12 bytes)."),
					maxlength=72,
					pattern='^([A-Fa-f0-9]{2})*$',
					type="text"
				},{
			...
			items={	{
				...
				},
				{
					...
					description = translate(
						'将untag的报文添加默认vlan（vlan范围:1-4094），填写"u"即可使用untag转untag模式（原光猫上网vlan为空的，可尝试填写）'
					),
					type = "text",
					maxlength = 4,
					depends = { ["互操作兼容模式"] = "3" }
				...
				},
				{
					...
					description = translate(
						"组播数据vlan（填写原光猫中的组播vlan，vlan范围:1-4094）"
					),
					type = "number",
					min = 1,
					max = 4094,
				...
				},
				{
					...
					description = translate(
						"将组播数据vlan转换为其他vlan（填写原光猫中iptv的vlan，即可将组播数据vlan转换为iptv认证vlan，vlan范围:1-4094）"
					),
					type = "number",
					min = 1,
					max = 4094,
				...
				},
				{
					...
					description = translate(
						'将下行vlan转换/绑定为其它vlan（可填写多组vlan转换对，填写示例："2:41,3:43,4:u,5:44@5"，即可将用户侧vlan：2、3、4、5分别转换为网络侧vlan：41、43、utag、44，其中"@5"表示指定网络侧vlan优先级，优先级范围:0-7，vlan范围:1-4094）'
					),
					type = "text",
					depends = { ["互操作兼容模式"] = "1" }
				},
				...
				},
				{
					...
					description = translate(
						"更改ME309规则的IGMP版本，基于IPoE的IPTV组播异常，可尝试更改"
					),
					type = "select",
					default = "3",
					options = {
		...
		},
		{
			...
			items = {
				...
				},
				{
					...
					description = translate(
						"Set the boot delay in seconds in which you can interupt the boot process over the serial console. With the Azores U-Boot, this also controls the number of times multicast upgrade is attempted and thus can have a significant impact in boot time. Default: 3, Recommended: 1"
					),
					type = "select_named",
					default = "3",
					base = true,
				...
				},
				{
					...
					description = translate(
						"Enable the serial console. This will cause TX_FAULT to be asserted as it shares the same SFP pin."
					),
					type = "checkbox",
					default = false,
				},
				{
					...
					description = translate(
						"Enable the serial console early in the boot process. Disabling this may help in some devices that have issues with TX_FAULT."
					),
					type = "checkbox_onoff",
					base = true,
					default = true,
				},
				{
					...
					description = translate(
						"Enable the RX_LOS pin. Disable to allow stick to be accessible without the fiber connected in all devices."
					),
					type = "checkbox",
					default = false,
				},
				{
					...
					description = translate(
						"Custom password hash for the root user. This can be set from System > Administration"
					),
					maxlength = 255,
				...
				},
				{
					...
					description = translate(
						"Number of seconds that we will delay the startup of omcid for at bootup (0 to 300). Defaults to 15 seconds"
					),
					type = "number",
					min = 0,
					max = 300,
				...
				},
				{
					...
					description = translate(
						"Allow the root file system to stay persistent (would also require that you modify the bootcmd fwenv). This is not recommended and should only be used for debug/testing purposes."
					),
					type = "checkbox",
					default = false,
				},
			...
			items = {
				{
					...
					description = translate(
						"Set the vendor name presented in the virtual EEPROM (up to 16 characters)."
					),
					type = "text",
					maxlength = 16,
				},
				{
					...
					description = translate(
						"Set the 3 byte vendor OUI presented in the virtual EEPROM (XX:XX:XX format)."
					),
					type = "text",
				...
				},
				{
					...
					description = translate(
						"Set the vendor part number presented in the virtual EEPROM (up to 16 characters)."
					),
					type = "text",
					maxlength = 16,
				},
				{
					...
					description = translate(
						"Set the vendor revision presented in the virtual EEPROM (up to 4 characters)."
					),
					type = "text",
					maxlength = 4,
				},
				{
					...
					description = translate(
						"Set the vendor serial number presented in the virtual EEPROM (up to 16 characters)."
					),
					type = "text",
					maxlength = 16,
				},
				...
				},
				{
					...
					description = translate(
						"Set the vendor specific data presented in the virtual EEPROM (up to 32 characters)."
					),
					type = "text",
					maxlength = 32,
				},
		...
		},
		{
			...
			items = {
				...
				},
				{
					...
					description = translate(
						"Enables a daemon that will ping an ip every 5 seconds, which can help with accessing the stick."
					),
					type = "checkbox",
					default = true,
				},
				{
					...
					description = translate(
						"IP address to ping. Defaults to the 2nd IP address in the configured management network (ie. 192.168.11.2)."
					),
					maxlength = 15,
					pattern = ipv4_regex,
					type = "text",
				...
				},
				{
					...
					description = translate(
						"Enables a reverse ARP monitoring daemon that will automatically add ARP entries from the MAC address of recieved packets."
				...
				},
				{
					...
					description = translate(
						"Automatically redirect requests to the WebUI over HTTP to HTTPs. Defaults to on."
					),
					type = "checkbox",
					default = true,
				},
...
function action_gpon_status()
	...
	local rv = {
		...
		temperature = string.format(
			"%s / %s / %s",
			temperature(metrics.cpu1_tempC),
			temperature(metrics.cpu2_tempC),
			temperature(metrics.optic_tempC)
		),
...
function action_vlan_extvlans()
	...
	if
		luci.sys.process.exec({ "/usr/sbin/8311-extvlan-decode.sh", "-t" }, luci.http.write, luci.http.write).code == 0
	then
		luci.http.write("\n\n")
		...
		for catid, cat in pairs(fwenvs) do
			for itemid, item in pairs(cat.items) do
				...
				if item.type == "checkbox" or item.type == "checkbox_onoff" then
					if
						item.value == ""
						and ((item.default and value == "1") or (not item.default and (value == "0" or value == "")))
					then
						value = ""
